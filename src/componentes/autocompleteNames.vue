<template>
    <div style="position:relative" v-bind:class="{'open':openSuggestion}">
        <input class="form-control" id="nombre" name="nombre" type="text" v-validate="'required'" :value="value" @input="updateValue($event.target.value)"
          @keydown.enter = 'enter'
          @keydown.down = 'down'
          @keydown.up = 'up'
        >
...
export default {
  ...
  methods: {
    ...
    enter () {
      this.$emit('input', this.matches[this.current].nombre)
      this.open = false
    },
    ...
    suggestionClick (index) {
      this.$emit('input', this.matches[index].nombre)
      this.open = false
    }
  }
}
